...
export async function GET() {
  try {
    ...
    const routineExercises = await db.all(`
      SELECT 
        re.id,
        re.routine_id,
        re.muscle_group_id,
    ...
    // La consulta SQL ahora se une a la tabla 'users' y concatena 'first_name', 'last_name',
    // 'week_number' y 'day_name' para crear el campo 'routine_info'.
...
export async function POST(request: Request) {
  try {
    ...
    const result = await db.run(
      'INSERT INTO routine_exercises (routine_id, muscle_group_id, exercise_id, series, weight, reps, rest_time, progress, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [routineId, muscleGroupId, exerciseId, series, weight, reps, restTime, progress, notes]
    );

    const newRoutineExercise = await db.get('SELECT * FROM routine_exercises WHERE id = ?', result.lastID);
