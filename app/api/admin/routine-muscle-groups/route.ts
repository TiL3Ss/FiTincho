...
export async function GET() {
  try {
    ...
    const routineMuscleGroups = await db.all(`
      SELECT 
        rmg.id,
        rmg.routine_id,
        rmg.muscle_group_id,
    ...
    `);
    // Nota: La consulta ahora combina 'first_name' y 'last_name' en el campo 'routine_info'.
...
export async function POST(request: Request) {
  try {
    ...
    const existingRelation = await db.get(
      'SELECT id FROM routine_muscle_groups WHERE routine_id = ? AND muscle_group_id = ?',
      [routine_id, muscle_group_id]
    );
    ...
    const result = await db.run(
      'INSERT INTO routine_muscle_groups (routine_id, muscle_group_id) VALUES (?, ?)',
      [routine_id, muscle_group_id]
    );

    const newRoutineMuscleGroup = await db.get(
